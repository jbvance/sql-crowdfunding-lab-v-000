...
FROM projects INNER JOIN pledges on projects.id = pledges.project_id GROUP BY projects.id HAVING amount_over >= 0"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) as total_amount FROM users INNER JOIN pledges on users.id = pledges.user_id GROUP BY users.id ORDER BY total_amount, users.name"
end
